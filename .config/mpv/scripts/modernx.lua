...
local user_opts = {
    ...
    titlefont = "ubuntu Nerd Font",             -- font used for the title above OSC and
...
local osc_styles = {
    ...
    bigButtons = "{\\blur0\\bord0\\1c&H404040&\\3c&HFFFFFF&\\fs58\\fnmodernx-osc-icon}",
    mediumButtons = "{\\blur0\\bord0\\1c&H404040&\\3c&HFFFFFF&\\fs36\\fnmodernx-osc-icon}",
    smallButtons = "{\\blur0\\bord0\\1c&H404040&\\3c&HFFFFFF&\\fs24\\fnmodernx-osc-icon}",

    timecodes1 = "{\\blur0\\bord0\\1c&HC2C3C5&\\3c&H000000&\\fs22}",
    timecodes2 = "{\\blur0\\bord0\\1c&H606162&\\3c&H000000&\\fs22}",
    ...
    elementHover = "{\\blur0\\1c&HBFBFBF&\\bord1}"
}

local osc_icons = {
    close = "\238\164\128",
    minimize = "\238\164\129",
    restore = "\238\164\130",
    maximize = "\238\164\131",
    ...
    playlist_prev = "\238\164\148",
    playlist_next = "\238\164\149",
    chapter_prev = "",
    chapter_next = "",
    play = "",
    pause = "",
...
function layout()
    ...
    lo = add_layout("bgBar")
    lo.geometry = {x = refX, y = refY - 90, an = 5, w = osc_geo.w - 50, h = 2}
    ...
    lo.geometry = {x = refX, y = refY - 90, an = 5, w = osc_geo.w - 50, h = 16}
    ...
    -- Title
    geo = {x = 25, y = refY - 110, an = 1, w = osc_geo.w - 10, h = 30}
    lo = add_layout("title")
    lo.geometry = geo
    ...
    lo.geometry = {x = 25, y = refY - 85, an = 7, w = tcW, h = 20}
    lo.style = osc_styles.timecodes1

    lo = add_layout("tc_middle")
    lo.geometry = {x = 103, y = refY - 85, an = 9, w = tcW, h = 20}
    lo.style = osc_styles.timecodes2

    lo = add_layout("tc_right")
    lo.geometry = {x = 176, y = refY - 85, an = 9, w = tcW, h = 20}
    lo.style = osc_styles.timecodes2
    ...
    if not volbar_cond then lo.geometry.x = osc_geo.w - 187 end
    ...
    if not volbar_cond then lo.geometry.x = osc_geo.w - 137 end
...
function osc_init()
    ...
    -- Separator
    ne = new_element("tc_middle", "button")

    ne.content = function ()
            if (state.tc_ms) then
                return (mp.get_property_osd("").."/")
            else
                return (mp.get_property_osd("").."/")
            end
        end
